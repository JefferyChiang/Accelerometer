...
def calibrate(data, mind, maxd, minm, maxm):
    dc = ((data - mind) * (maxm-minm) / (maxd-mind) + mapmin)/mapmin
    return dc
...
def freq_filter(data, f_size, cutoff):
	...
	for i in range(num_signal):
		f_data[:,i]=signal.convolve(data.iloc[:,i], lpf, mode='same')
	return f_data
...
def center(data_filter):
    ...
    data_center = pd.DataFrame([data_filter[0]-x_mean, data_filter[1]-y_mean, data_filter[2]-z_mean])
...
comb_data=freq_filter(pd.DataFrame(median_data), numtaps, cutoff/fs)
plot_raw(pd.DataFrame(comb_data[100:-100]), fs)
...
res = combine_threshold(data_center)
plot_raw(pd.DataFrame(res), fs)
...
#print(res)
# 積分運算
freq1 = 0.0035 #for example freq=10 if you have 10 records per second
freq2 = 0.11
freq3 = 0.012
# 9800 mm/s^2
...
plot_v(v_x_go, 0.1, 'Vx(mm/s)')
plot_v(v_y_go, 0.1, 'Vy(mm/s)')
plot_v(v_z_go, 0.1, 'Vz(mm/s)')
...
plot_d(d_x_put, 0.1, 'Dx(mm)')
plot_d(d_y_go, 0.1, 'Dy(mm)')
plot_d(d_z_put, 0.1, 'Dz(mm)')
